...
void Application::ProcessMouseMovement(sf::Event a_event)
{
	...
	m_v3MousePos.x = static_cast<float>(mouse.x - window.x);
	m_v3MousePos.y = static_cast<float>(mouse.y - window.y);
	if(!m_pSystem->IsWindowFullscreen() && !m_pSystem->IsWindowBorderless())
		m_v3MousePos += vector3(-8.0f, -32.0f, 0.0f);
	gui.io.MousePos = ImVec2(m_v3MousePos.x, m_v3MousePos.y);
}
...
void Application::ProcessJoystickPressed(sf::Event a_event)
{
	...
	if (m_pController[m_uActiveController]->button[SimplexKey_L3] && m_pController[m_uActiveController]->button[SimplexKey_R3])
		m_bRunning = false;
}
void Application::ProcessJoystickReleased(sf::Event a_event)
{
	...
	if (m_pController[m_uActiveController]->mapButton[nButton] == SimplexKey_Pad)
		m_bRunning = false;
}
...
#pragma region Camera Position
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::W))
		m_pCameraMngr->MoveForward(m_fCameraMovementSpeed * fMultiplier);

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::S))
		m_pCameraMngr->MoveForward(-m_fCameraMovementSpeed * fMultiplier);

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::A))
		m_pCameraMngr->MoveSideways(-m_fCameraMovementSpeed * fMultiplier);

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::D))
		m_pCameraMngr->MoveSideways(m_fCameraMovementSpeed * fMultiplier);

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Q))
		m_pCameraMngr->MoveVertical(-m_fCameraMovementSpeed * fMultiplier);
...
void Application::ProcessJoystick(void)
{
	...
	*/
#pragma region Camera Position
	float fForwardSpeed = m_pController[m_uActiveController]->axis[SimplexAxis_Y] / 150.0f;
	float fHorizontalSpeed = m_pController[m_uActiveController]->axis[SimplexAxis_X] / 150.0f;
	float fVerticalSpeed = m_pController[m_uActiveController]->axis[SimplexAxis_R] / 150.0f -
		m_pController[m_uActiveController]->axis[SimplexAxis_L] / 150.0f;

	bool fMultiplier = m_pController[m_uActiveController]->button[SimplexKey_L1] ||
		m_pController[m_uActiveController]->button[SimplexKey_R1];
...
#pragma region Camera Orientation
	//Change the Yaw and the Pitch of the camera
	m_pCameraMngr->ChangeYaw(-m_pController[m_uActiveController]->axis[SimplexAxis_U] / 150.0f);
	m_pCameraMngr->ChangePitch(m_pController[m_uActiveController]->axis[SimplexAxis_V] / 150.0f);
#pragma endregion
#pragma region ModelOrientation Orientation
	m_qArcBall = quaternion(vector3(glm::radians(m_pController[m_uActiveController]->axis[SimplexAxis_POVY] / 20.0f), 0.0f, 0.0f)) * m_qArcBall;
	if (fMultiplier)
	{
		m_qArcBall = quaternion(vector3(0.0f, 0.0f, glm::radians(m_pController[m_uActiveController]->axis[SimplexAxis_POVX] / 20.0f))) * m_qArcBall;
	}
	else
	{
		m_qArcBall = quaternion(vector3(0.0f, glm::radians(m_pController[m_uActiveController]->axis[SimplexAxis_POVX] / 20.0f), 0.0f)) * m_qArcBall;
	}
#pragma endregion
}